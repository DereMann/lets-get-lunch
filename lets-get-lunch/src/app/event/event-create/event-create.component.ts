...
export class EventCreateComponent implements OnInit {
  ...
  error: string;
  success: string;
  @ViewChild('city') citySearch: ElementRef; //reference to our view

  constructor(private fb: FormBuilder, private gmaps: MapsAPILoader, private ngZone: NgZone,
  ...
  ngOnInit() {
    ...
    this.gmaps.load().then(() => {
      ...
      autocomplete.addListener('place_changed', () => {
        this.ngZone.run(() => {
          this.location = autocomplete.getPlace(); //add a console.log() below the return value we get from the autocomplete.
        });
      });
    });
  }
